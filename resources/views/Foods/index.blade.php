    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Manajemen Makanan</h3>
                    <p class="text-sm text-gray-500">Daftar semua Makanan yang terdaftar di aplikasi FoodMarket.</p>
                </div>
                <a href="{{ route('food.create') }}" class="inline-flex items-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-lg transition duration-200 ease-in-out transform hover:-translate-y-0.5">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                ...
                                <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Types</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50/75 transition duration-150">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600">{{ number_format($u->price) }}</td>
                                <td class="px-6 py-4">{{ $u->rate  }}</td>
                                <td class="px-6 py-4">{{ $u->types  }}</td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('food.edit', $u->id) }}" class="inline-flex items-center px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg shadow-sm hover:shadow transition duration-150">
                                        <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                        </svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('food.destroy', $u->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?');">
                                        @csrf
                                        @method('DELETE')
